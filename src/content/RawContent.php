...
final class RawContent
{
    ...
    public function modifyFieldValues(array $map, callable $transform, array $options = []): array
    {
        ...
        while (true) {
            ...
            $contentContains = $options['contentContains'] ?? null;
            if ($contentContains !== null && $contentContains !== '') {
                $query->andWhere(['like', new Expression('CAST([[s.content]] AS ' . ($db->getDriverName() === 'pgsql' ? 'TEXT' : 'CHAR') . ')'), $contentContains]);
            }
            $rows = $query->all($db);
            if (!$rows) {
                break;
            ...
            foreach ($rows as $row) {
                ...
                // Compare the stored representation: associative decoding would hide
                // a concurrent change between an empty object and an empty array.
    ...
    private function _transform(mixed $value, string $fieldUid, array $map, callable $transform, array $context, array &$stats, int $depth): mixed
    {
        ...
        return $adapter->transform($value, $definition['schema'], function(mixed $raw, array $placement, array $segment) use ($fieldUid, $map, $transform, $context, &$stats, $depth): array {
            ...
            if ($change['action'] === 'remove' || $change['value'] !== $nested) {
                $stats['changed']++;
            }
            return $change;
        });
