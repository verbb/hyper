 ...
 */
function docsScreenshotLayoutConfig(?PlainText $extraField = null, int $tabIndex = 0): array
{
    $layout = Url::getDefaultFieldLayout();

    if ($extraField) {
        $tabs = $layout->getTabs();
        $contentTab = $tabs[$tabIndex] ?? null;
...
function docsScreenshotLinkTypeConfig(
    ...
    $link = \verbb\hyper\Hyper::$plugin->getLinks()->createLink(array_merge([
        'type' => $type,
        'label' => $label,
        'handle' => $handle,
        'enabled' => $enabled,
...
function docsScreenshotHyperField(PlainText $subtitleField): HyperField
{
    ...
    $urlHandle = 'default-verbb-hyper-links-url';
    $entryHandle = 'default-verbb-hyper-links-entry';
    // Keep the primary input compact while demonstrating project fields in the settings slide-out.
    $defaultLayout = docsScreenshotLayoutConfig($subtitleField, 1);
    ...
    $linkTypes = [
        docsScreenshotLinkTypeConfig(Asset::class, 'Asset', 'default-verbb-hyper-links-asset', false),
        docsScreenshotLinkTypeConfig(Category::class, 'Category', 'default-verbb-hyper-links-category', true),
        docsScreenshotLinkTypeConfig(Custom::class, 'Custom', 'default-verbb-hyper-links-custom', false),
        docsScreenshotLinkTypeConfig(Email::class, 'Email', 'default-verbb-hyper-links-email', false),
        docsScreenshotLinkTypeConfig(Embed::class, 'Embed', 'default-verbb-hyper-links-embed', false),
        ...
        docsScreenshotLinkTypeConfig(User::class, 'User', 'default-verbb-hyper-links-user', false),
        // Keep remaining registered types present but off so settings UI does not re-append them as enabled.
        docsScreenshotLinkTypeConfig(Phone::class, 'Phone', 'default-verbb-hyper-links-phone', false),
        docsScreenshotLinkTypeConfig(SiteLink::class, 'Site', 'default-verbb-hyper-links-site', false),
    ];
...
$demo = docsScreenshotUpsertEntry($hyperSection, DOCS_ENTRY_HANDLE, 'Demo', [
    $field->handle => [
        [
            ...
            'newWindow' => true,
            'fields' => [
                DOCS_SUBTITLE_HANDLE => 'Plan a brilliant day out',
            ],
        ],
        [
...
// Saving nested Hyper content can clear the generated title in this disposable fixture.
if ($demo->title !== 'Demo') {
    $demo->title = 'Demo';

    if (!Craft::$app->getElements()->saveElement($demo)) {
        throw new RuntimeException('Unable to retitle Demo entry: ' . Json::encode($demo->getErrors()));
    }
}
